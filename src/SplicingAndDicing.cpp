...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    if (target == nullptr || dna == nullptr){
        return dna;
    }else{
        Nucleotide* result = dna;
        Nucleotide* target_lagger = target;
        while(dna != nullptr){
            while(dna->value == target->value){
                dna = dna->next;
                target = target->next;
                if (target == nullptr){
                    return result;
...
void blowItUp(Nucleotide*& bomber);

bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    //the job is done if target is empty
    if (target == nullptr) {
        return true;
    }
    //nothing to search if dna is empty
    if (dna == nullptr){
        return false;
    }else{
        Nucleotide* bomber;
        Nucleotide* head = dna;
        Nucleotide* dna_counter = dna;
        Nucleotide* target_lagger = target;
        while(head != nullptr){
            //a match is found, let's see if it's a complete match
            while(head->value == target->value){
                head = head->next;
                target = target->next;
                //the case of successfully finding splice sequence
                if (target == nullptr){
                    //make sure last nucleo in seq has next pointing to nullpointer
                    //otherwise blowItUp will not end
                    bomber->next = nullptr;
                    bomber = dna_counter;
                    //if the seq is at the very beginning, move dna pointer before deletion
                    if (dna_counter == dna){
                        bomber = dna;
                        dna = head;
                        dna_counter = dna_counter->prev;
                        //if the seq is not at the beginning, make sure first nucleo in seq has prev pointing to nullpointer
                    } else { 
                        dna_counter = dna_counter->prev;
                        bomber->prev = nullptr;
                    }
                    //move the pointers to exclude the seq
                    if (dna_counter != nullptr){
                        dna_counter->next = head;
                    }
                    if (head != nullptr){
                        head->prev = dna_counter;
                    }
                    //delete the seq
                    blowItUp(bomber);
                    return true;
...
//Helper function, deletes the excised portion of DNA
void blowItUp(Nucleotide*& bomber) {
    Nucleotide* grimreaper = bomber;
    while(grimreaper != nullptr){
        grimreaper = bomber->next;
        delete bomber;
        bomber = grimreaper;
    }
    return;
